...
public class NewPlayer_fixed implements IPlayer {
	 ...
	 * x4 = 5 marker in a line of our player
	 * x5 = Single marker in a line of the enemy player 
	 * x6 = 2 marker in a line of the enemy player 
	 * x7 = 3 marker in a line of the enemy player 
	 * x8 = 4 marker in a line of the enemy player
	 * x9 = 5 marker in a line of the enemy player
	 */
	...
	/**
	 * computes V(x1,x2,...,x10) = w1*x1 + w2*x2 + ... + w10*x10
	 * 
	 * @param features
	 *            the x values (which are representing the different features)
	 * @return the result of V(x1,...,x10)
	 */
	private double computeValue(double[] features) {
		double sum = 0;
	...
	private double computeValueAtPosition(IBoard board, int x, int y, int z) {
		...
		double[] currentFeatures = getFeatures(copy);
		...
		double[] futureFeatures = getFeatures(copy);
	 ...
	 */
	private double[] getFeatures(IBoard board) {
	...
	private void updateWeights(IBoard board) {
		...
		while (!history.isEmpty()) {
			...
			double[] features = getFeatures(reconstuctedBoard);

			vLearned = computeValue(features);
			
			double error = vTrain - vLearned;

			//System.out.println("learned " + vLearned + " train " + vTrain + " error " + error);
			//System.out.println("zugehörige Features: " + Arrays.toString(features));
			
			for (int i = 0; i < FEATURE_LENGTH; i++) {
				weights[i] = weights[i] + ETA * features[i] * error;
			}
			//System.out.println("neue weights: " + Arrays.toString(weights));
